...
with main_content_col:
    ...
    start_analysis = render_analysis_controls(
        analysis_params, llm_config, selected_analysts
    )
    ...
    if start_analysis:
        ...
        st.session_state.current_max_debate_rounds = analysis_params[
            "max_debate_rounds"
        ]
        ...
        if not openai_key or not finnhub_key:
            st.error(
                "Please configure both OpenAI and Finnhub API keys in your .env file"
            )
            st.stop()
    ...
    stock_symbol = st.session_state.get(
        "current_stock_symbol", analysis_params.get("stock_symbol", "NVDA")
    )
    analysis_date = st.session_state.get(
        "current_analysis_date", analysis_params.get("analysis_date")
    )
    max_debate_rounds = st.session_state.get(
        "current_max_debate_rounds", analysis_params.get("max_debate_rounds", 2)
    )
    max_risk_rounds = st.session_state.get(
        "current_max_risk_rounds", analysis_params.get("max_risk_rounds", 2)
    )
    online_tools = st.session_state.get(
        "current_online_tools", analysis_params.get("online_tools", True)
    )
    debug_mode = st.session_state.get(
        "current_debug_mode", analysis_params.get("debug_mode", False)
    )
    ...
    if st.session_state.analysis_running:
        ...
        st.session_state.agent_progress_placeholder.progress(
            progress,
            text=f"Progress: {completed_agents}/{total_agents} agents completed",
        )
...
with agent_status_col:
    ...
    if st.session_state.get("current_debug_mode", False):
        ...
        st.write(
            "Debug - Analysis Has Started:",
            st.session_state.get("analysis_has_started", False),
        )
    ...
    show_agent_status = (
        ...
        or any(status != "pending" for status in st.session_state.agent_status.values())
        or st.session_state.get("analysis_has_started", False)
        or True  # Force show for debugging
    )
    ...
    if show_agent_status:
        ...
        for team_name, agents in teams.items():
            completed = sum(
                1
                for agent in agents
                if st.session_state.agent_status[agent] == "complete"
            )
            total = len(agents)

            with st.expander(
                f"{team_name} ({completed}/{total} complete)", expanded=True
            ):
        ...
        if st.session_state.analysis_results and not st.session_state.analysis_running:
            ...
            if final_state.get("market_report"):
                if st.button(
                    "📈 Market Analysis", use_container_width=True, key="market_btn"
                ):
            ...
            if final_state.get("news_report"):
                if st.button(
                    "📰 News Analysis", use_container_width=True, key="news_btn"
                ):
            ...
            if final_state.get("fundamentals_report"):
                if st.button(
                    "💰 Fundamentals", use_container_width=True, key="fundamentals_btn"
                ):
            ...
            if final_state.get("trader_investment_plan"):
                if st.button(
                    "⚖️ Risk Assessment", use_container_width=True, key="risk_btn"
                ):
    ...
    if st.session_state.get("show_agent_details") and st.session_state.get(
        "selected_agent"
    ):
            ...
            if agent_status == "running":
                ...
                if "Market" in selected_agent:
                    with st.container():
                        ...
                        message_index = (current_time // 3) % len(
                            progress_messages
                        )  # Change every 3 seconds

                        st.markdown(
                            f"**Current Task:** {progress_messages[message_index]}"
                        )
                ...
                # Add support for other agent types
                    ...
                    "Risk" in selected_agent
                    or "Conservative" in selected_agent
                    or "Aggressive" in selected_agent
                ):
                    with st.container():
                ...
                if st.session_state.analysis_results:
                    ...
                    if "Market" in selected_agent and final_state.get("market_report"):
                        ...
                        st.markdown(
                            final_state["market_report"][:500] + "..."
                            if len(final_state["market_report"]) > 500
                            else final_state["market_report"]
                        )

                        with st.expander("📈 View Full Market Report"):
                            st.markdown(final_state["market_report"])

                    elif "Social" in selected_agent and final_state.get(
                        "sentiment_report"
                    ):
                        st.markdown("#### 👥 Social Analysis Results:")
                        st.markdown(
                            final_state["sentiment_report"][:500] + "..."
                            if len(final_state["sentiment_report"]) > 500
                            else final_state["sentiment_report"]
                        )
                        ...
                        st.markdown(
                            final_state["news_report"][:500] + "..."
                            if len(final_state["news_report"]) > 500
                            else final_state["news_report"]
                        )

                        with st.expander("📰 View Full News Report"):
                            st.markdown(final_state["news_report"])

                    elif "Fundamentals" in selected_agent and final_state.get(
                        "fundamentals_report"
                    ):
                        st.markdown("#### 💼 Fundamentals Analysis Results:")
                        st.markdown(
                            final_state["fundamentals_report"][:500] + "..."
                            if len(final_state["fundamentals_report"]) > 500
                            else final_state["fundamentals_report"]
                        )
                ...
                st.markdown(
                    "This agent will begin analysis once the previous agents complete their tasks."
                )
...
if st.session_state.analysis_running:
    ...
    if selected_analysts_from_sidebar:
        active_selected_analysts = [
            analyst_mapping.get(key, key) for key in selected_analysts_from_sidebar
        ]
    else:
    ...
    try:
        ...
        def _has_price_data(
            ...
            try:
                ...
                return not (
                    data.get("s") == "no_data"
                    or (isinstance(data.get("c"), list) and not data.get("c"))
                )
            except Exception:
                return True

        market_selected = any("Market Analyst" in a for a in active_selected_analysts)
        if market_selected and not _has_price_data(
            stock_symbol, analysis_date - timedelta(days=60), analysis_date
        ):
    ...
    try:
        ...
        ta = TradingAgentsGraph(
            selected_analysts=active_selected_analysts, debug=debug_mode, config=config
        )
...
if st.session_state.get("last_error") and not st.session_state.analysis_running:
    st.error(
        f"An error occurred during the last analysis: {st.session_state.last_error}"
    )

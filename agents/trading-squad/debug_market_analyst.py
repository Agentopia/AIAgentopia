    ...
    try:
        ...
        if hasattr(ta, "graph"):
            ...
            if hasattr(ta.graph, "nodes"):
                logger.info(
                    f"🔗 Graph nodes: {list(ta.graph.nodes.keys()) if hasattr(ta.graph.nodes, 'keys') else 'Unknown'}"
                )
...
try:
    ...
    print(
        "📋 Check 'market_analyst_debug.log' for execution details up to interruption"
    )

...
with st.sidebar:
    ...
    if llm_provider == "openai":
        ...
        quick_think_model = st.selectbox(
            "Quick Think Model", options=["gpt-4o-mini", "gpt-3.5-turbo"], index=0
        )
        backend_url = "https://api.openai.com/v1"
    else:  # google
        deep_think_model = st.selectbox(
            "Deep Think Model", options=["gemini-2.0-flash", "gemini-1.5-pro"], index=0
        )
        quick_think_model = st.selectbox(
            "Quick Think Model",
...
with col1:
    ...
    analysis_date = st.date_input(
        "Analysis Date", value=date.today(), help="Select the date for analysis"
    )
